...
openssl genrsa -out ${NAME_CRT}.key ${KEY_SIZE}

echo -e "Creation CSR"
openssl req -new -sha512 -key ${NAME_CRT}.key -nodes -out ${NAME_CRT}.csr -subj "${CRT_SUBJ}"

echo -e "Creation et signature du CRT"
openssl x509 -req -sha512 -days 36500 -in ${NAME_CRT}.csr -CA ${NAME_ROOT_CA}.crt -CAkey ${NAME_ROOT_CA}.key -CAcreateserial -out ${NAME_CRT}.crt
